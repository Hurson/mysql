	...
	public boolean sendLocalFile(String sourceFile,String targetPath)
	{
		...
		for (int i=0;i<cpsList.size();i++)
		{
			try {
				...
				warnHelper.writeWarnMsgToDb("双向广告FTP服务器无法连接   ip:" + cpsList.get(i).getIp() + ", port: " + Integer.parseInt(cpsList.get(i).getPort()) + " , user: " + cpsList.get(i).getUser() + ", pwd: " + cpsList.get(i).getPwd());
				return false;
			} 
			//copy本地文件到资源服务上
