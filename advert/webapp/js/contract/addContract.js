		...
		setTimeout(function () {
                $(elementsName).attr('onclick', '').unbind('click').bind('click', function () {
					...
					easyDialog.open({
						container : {
							header : '绑定规则',
							content : generateStructContract('bindingMarketingRule',positionId,chooseMarketRulesElement,currentIndex,positionIndexFlagParam)
						},
						overlay : false
					});
		...
		setTimeout(function () {
                $(elementsName).attr('onclick', '').unbind('click').bind('click', function () {
					...
					easyDialog.open({
						container : {
							header : '查看并修改已绑定规则',
							content : generateStructContract('viewAlreadyBindingPosition',positionId,chooseMarketRulesElement,currentIndex,positionIndexFlagParam)
						},
						overlay : false,
...
function generateStructContract(methodName,positionId,chooseMarketRulesElement,currentIndex,positionIndexFlagParam){
	...
	if(methodName=='bindingCustomer'){
		...
		structInfo+='					<div style="margin:0px;padding:0px;">';
		structInfo+='							<iframe id="bingPositionFrame" name="bingPositionFrame" src="'+resourcePath+'/page/position/queryPositionPage.do?method=queryPage&contractBindingFlag=1'+'&positionIndexFlag='+positionIndexFlagParam+'&saveOrUpdateFlag=save" frameBorder="0" width="1000px" height="400px"  scrolling="yes"></iframe>';
		structInfo+='					</div>';
...
function compareModifyRecord(beforeRecord,afterRecord){
	...
	if((!$.isEmptyObject(beforeRecord))&&(!$.isEmptyObject(afterRecord)))
	{
		...
		comparedForm.bindingPosition='[';
		/*$(afterRecord.bindingPosition).each(function(indexAfter,itemAfter){
			comparedForm.bindingPosition+='{';
			
			comparedForm.bindingPosition+='}'
			alert(comparedForm.bindingPosition);
			$(beforeRecord.bindingPosition).each(function(indexBefore,itemBefore){
...
function validateDate(){
	...
	if(alreadyFillInForm.bindingPosition.length<=0){
		...
		if($.isEmptyObject(customerIdVal.trim())){
				flag = false;
				createDialog('请绑定广告商');
				return flag;
		}
		...
		var contractNumberVal=$('#contractNumber').val();
		if($.isEmptyObject(contractNumberVal.trim())){
				flag = false;
		...
		var contractCodeVal=$('#contractCode').val();
		if($.isEmptyObject(contractCodeVal.trim())){
				flag = false;
		...
		var contractNameVal=$('#contractName').val();
		if($.isEmptyObject(contractNameVal.trim())){
				flag = false;
		...
		var submitUnitsVal=$('#submitUnits').val();
		if($.isEmptyObject(submitUnitsVal.trim())){
				flag = false;
		...
		var financialInformationVal=$('#financialInformation').val();
		if($.isEmptyObject(financialInformationVal.trim())){
				flag = false;
		...
		var approvalCodeVal=$('#approvalCode').val();
		if($.isEmptyObject(approvalCodeVal.trim())){
				flag = false;
		...
		var effectiveStartDateVal=$('#effectiveStartDate').val();
		if($.isEmptyObject(effectiveStartDateVal.trim())){
				flag = false;
		...
		var effectiveEndDateVal=$('#effectiveEndDate').val();
		if($.isEmptyObject(effectiveEndDateVal.trim())){
				flag = false;
		...
		var approvalStartDateVal=$('#approvalStartDate').val();
		if($.isEmptyObject(approvalStartDateVal.trim())){
				flag = false;
		...
		var approvalEndDateVal=$('#approvalEndDate').val();
		if($.isEmptyObject(approvalEndDateVal.trim())){
				flag = false;
		...
		var otherContentVal=$('#otherContent').val();
		if($.isEmptyObject(otherContentVal.trim())){
				flag = false;
				createDialog('其他内容不能为空');
				return flag;
		}
	
		var descVal=$('#desc').val();
		if($.isEmptyObject(descVal.trim())){
				flag = false;
			...
			if(positionList.length>1){
				...
				$(positionList).each(function(positionIndexInner,positionInnerItem){
					...
					$(positionList).each(function(positionOuterIndex,positionOuterItem){
						// 1、日期不能交叠
						if(positionOuterItem.validStartDate>=startDate&&positionOuterItem.validStartDate<=endDate){
							flag = false;
							createDialog('所绑定开始时间有重叠');
							return flag;
						}
						
						if(positionOuterItem.validEndDate>=startDate&&positionOuterItem.validEndDate<=endDate){
							flag = false;
							createDialog('所绑定结束时间有重叠');
							return flag;
						}
						
						if(positionOuterItem.validStartDate<=startDate&&(positionOuterItem.validEndDate>=startDate||positionOuterItem.validEndDate>=endDate)){
							flag = false;
							createDialog('所绑定结束时间有重叠');
							return flag;
						}
