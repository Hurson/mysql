...
/*
 * Grid  2.0
 * Copyright (c) 2009, Yangjunhai
 * Extends jqGrid  3.5.3 (c) 2008, Tony Tomov, tony@trirand.com
 * Date: 2009-10-26
 */
$.jgrid = $.jgrid || {};
$.extend($.jgrid,{  
...
$.fn.jqGrid = function( p ) {
	...
	return this.each( function() {
		...
		setSelarrrow = function () {
            if(ts.p.telselarrrow==undefined) return;
			var newselarrrow = ts.p.telselarrrow;
            var snewselarrrow=[];
            for (var i = 0; i < newselarrrow.length; i++) {
              	$("#gview_"+ts.p.id).find("tr#"+ newselarrrow[i]).addClass("ui-state-highlight");
                $("#gview_"+ts.p.id).find("tr#"+ newselarrrow[i]).attr("aria-selected",true);
                $("#gview_"+ts.p.id).find("#jqg_"+ newselarrrow[i]).attr("checked", true);
			    snewselarrrow[i]=newselarrrow[i];
            }
			ts.p.selarrrow=snewselarrrow;
        },
		...
		addJSONData = function(data,t, rcnt) {
			...
			ts.p.telselarrrow=new Array();
			for(var i=0;i<ts.p.selarrrow.length;i++){
			 ts.p.telselarrrow[i]=ts.p.selarrrow[i];
			}
			...
			if(ts.p.userDataOnFooter) $(ts).footerData("set",ts.p.userData,true);
			updatepager(false);
            if($(grid.bDiv)[0].clientHeight > $(grid.bDiv)[0].scrollHeight){
              $("#" + ts.p.id).reloadGridWidthByRef(0);
            }
		},
				...
				case "script":
					$.ajax({url:ts.p.url,type:ts.p.mtype,dataType: dt ,data: ts.p.postData,
						complete:function(req,st) {
                            if(ts.p.beforeComplete){
                              ts.p.beforeComplete(req,st);
                            }
		...
		setPager = function (){
			...
			if(ts.p.pageInfo)$("td#"+pgid+"_"+ts.p.pagerpos,"#"+pgcnt).append(pgl);
		...
		setColWidth = function () {
			var initwidth = 0, brd=ts.p.cellLayout, vc=0, lvc, scw=ts.p.scrollOffset,cw,hs=false,aw,taw,tw=0,gw=0,
			msw = ts.p.multiselectWidth, sgw=ts.p.subGridWidth, rnw=ts.p.rownumWidth, cl = ts.p.cellLayout, cr;
			if(ts.p.tempColModel==undefined){
				ts.p.tempColModel=new Array();
			    $.each(ts.p.colModel, function(i) {
				  ts.p.tempColModel[i]=this.width;
			    });
			};
			$.each(ts.p.colModel, function(i) {
			...
			if(ts.p.shrinkToFit===true) {
                ...
                if(ts.p.tableWidth>grid.width){
				 taw = ts.p.tableWidth-brd*vc-gw;
                }
				...
				$.each(ts.p.colModel, function(i) {
					if(this.hidden === false && this.name !== 'cb' && this.name !== 'subgrid' && this.name !== 'rn'){
						...
						initwidth += cw;
                        if(ts.p.tableWidth>grid.width){
				          cw = Math.floor(taw/(ts.p.tblwidth-tw)*this.width);
                        }
						this.width =cw;
						lvc = i;
					}
        ...
        if(ts.p.tableWidth==undefined)ts.p.tableWidth=ts.p.width;
		if(ts.p.autowidth===true) {
			var pw = $(eg).innerWidth();
			ts.p.width = pw > 0?  pw: ts.p.width;
		}
        grid.tempwidth=ts.p.width;
		setColWidth();
		...
		$("table:first",grid.bDiv).addClass("mygrid_ui_to_num");
		if( isMSIE ) {
...
$.fn.extend({
	...
	setSelection : function(selection,onsr) {
		return this.each(function(){
			...
			if(!$t.p.multiselect) {
                ...
                if($t.p.clickRow)$t.p.clickRow(selection,stat);
			} else {
				$t.p.selrow = selection;
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			var $t = this, cw,
			initwidth = 0, brd=$t.p.cellLayout, lvc, vc=0, isSafari,hs=true, scw=$t.p.scrollOffset, aw,taw, gw=0, tw=0,
			msw = $t.p.multiselectWidth, sgw=$t.p.subGridWidth, rnw=$t.p.rownumWidth, cl = $t.p.cellLayout,cr;
			if (!$t.grid ) {return;}
			if(typeof shrink != 'boolean') {
				shrink=$t.p.shrinkToFit;
			}
            $t.grid.tempwidth=nwidth;
		...
		//	if(nwidth == $t.grid.width) {return;}
          ...
          //  }
			...
			if(shrink===true) {
			$.each($t.p.tempColModel, function(i) {
				  $t.p.colModel[i].width=this;
			});
                ...
                if($t.p.tableWidth>$t.grid.width){
				 taw = $t.p.tableWidth-brd*vc-gw;
                }
				 aw = nwidth-brd*vc-gw;
				if(!isNaN($t.p.height)) {
				...
				$.each($t.p.colModel, function(i) {
					...
					if(this.hidden === false && tn !== 'cb' && tn !== 'subgrid' && tn !== 'rn'){
						...
						initwidth += cw;
                        if($t.p.tableWidth>$t.grid.width){
				          cw = Math.floor(taw/($t.p.tblwidth-tw)*this.width);
                        }
						this.width =cw;
				...
				if(cl>0&&$t.grid.cols[lvc]!=undefined) $t.grid.cols[lvc].style.width = cw+"px";
            ...
            if($("#gview_"+$t.p.id+">.ui-jqgrid-hdiv>.ui-jqgrid-hbox>.ui-jqgrid-htable").innerWidth()+1==$("#gbox_"+$t.p.id).innerWidth()){
              $("#gview_"+$t.p.id+">.ui-jqgrid-hdiv>.ui-jqgrid-hbox>.ui-jqgrid-htable").css('width',$("#gbox_"+$t.p.id).innerWidth()+'px');
              $("#gview_"+$t.p.id+">.ui-jqgrid-bdiv>.ui-jqgrid-btable").css('width',$("#gbox_"+$t.p.id).innerWidth()+'px');
            }
		});
	},
    ...
    reloadGridWidthByRef : function(time) {
       return this.each(function(){
			var $t = this;
		    window.setTimeout(function() {
            var width=0;
             for(var i=0;i<$t.p.widthref.length;i++){
                 width = $($t.p.widthref[i]).innerWidth();
                 if(width >0){
                      break;
                 }
             } 
             $("#" + $t.p.id).setGridWidth(width);  
             }, time);
       });
	},
