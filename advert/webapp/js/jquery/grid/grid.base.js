...
(function ($) {
    ...
    $.fn.jqGrid = function(p) {
        ...
        $.fn.setSelarrrow = function (newselarrrow) {
            this.each(function() {
                var $t = this;
                if (newselarrrow == undefined) {
                    newselarrrow = $t.p.selarrrow
                }
//                if(newselarrrow instanceOf String){}
                for (var i = 0; i < newselarrrow.length; i++) {
                //alert($("tr#" + newselarrrow[i]));
                    $("tr#" + newselarrrow[i]).addClass("selected");
                    $("#jqg_"+ newselarrrow[i]).attr("checked", true)
                }
            });
        };
        ...
        $.fn.addRowData = function(rowid, data, pos, src) {
            ...
            if (data) {
                this.each(function() {
                    ...
                    try {
                        t.p.afterInsertRow(row.id, data);
                    } catch(e) {
                    }
                    t.updatepager();
                    success = true;
        ...
        $.fn.setGridWidth = function(nwidth, shrink) {
            return this.each(function() {
                ...
                if (shrink !== true) {
                    testdata[0] = Math.min($t.p._width, $t.grid.width);
                    testdata[2] = 0;
                }
                else {
                ...
                $.each($t.p.colModel, function(i, v) {
                    ...
                    if (this.name == 'cb' || this.name == 'subgrid') {
                        chw += IENum(this.width);
                    }
                });
                if (chw + testdata[1] <= nwidth || $t.p.forceFit === true) {
                    ofl = "hidden";
                    tw = nwidth;
                }
                else {
        ...
        $.fn.setGridHeight = function (nh) {
            return this.each(function () {
                ...
                if ($t.p.forceFit === true) {
                    ...
                    ovfl2 = $($t.grid.bDiv).css("overflow-x");
                }
        ...
        return this.each(function() {
            ...
            var addJSONData = function(data, t, rcnt) {
                ...
                ts.p.selarrrow = data[ts.p.jsonReader.check] || {};
                ...
                if (drows) {
                    for (i = 0; i < drows.length; i++) {
                        ...
                        if (ts.p.subGrid) {
                            try {
                                $(ts).addSubGrid(t, row, gi, drows[i]);
                            } catch (e) {
                            }
                            si = 1;
                        }
                        ...
                        if (afterInsRow) {
                            ts.p.afterInsertRow(row.id, rd, drows[i]);
                        }
                        rd = [];
                    }
                }
            ...
            var updatepager = function() {
                if (ts.p.pager) {
                    ...
                    if (ts.p.viewrecords) {
                        if (ts.p.pgtext) {
                            $('#sp_1', ts.p.pager).html(ts.p.pgtext + "&#160;" + ts.p.lastpage + "&#160;" + ts.p.pageStr);
                        }
                        $('#sp_2', ts.p.pager).html("&#160;&#160;&#160;&#160;" + ts.p.allre + "&#160;" + ts.p.records + "&#160;" + ts.p.recordtext + "&#160;");
                    }
            ...
            var populate = function () {
                if (!grid.hDiv.loading) {
                    ...
                    switch (ts.p.datatype)
                            {
                        case "json":
                            $.ajax({url:ts.p.url,type:ts.p.mtype,dataType:"json",data: gdata, complete:function(JSON, st) {
                                if (st == "success") {
                                    addJSONData(eval("(" + JSON.responseText + ")"), ts.grid.bDiv, rcnt);
                                    JSON = null;
                                    if (loadComplete) {
                                        loadComplete();
                                    }
                            ...
                            break;
                        case "xml":
                            $.ajax({url:ts.p.url,type:ts.p.mtype,dataType:"xml",data: gdata , complete:function(xml, st) {
                                if (st == "success") {
                                    addXmlData(xml.responseXML, ts.grid.bDiv, rcnt);
                                    xml = null;
                                    if (loadComplete) {
                                        loadComplete();
                                    }
            ...
            var setColWidth = function () {
                ...
                if (self.parent != undefined && self.parent.document != undefined&& self.parent.document.all != undefined&& self.parent.document.all.TB_iframeContent != undefined) {
                    var ifwidth = self.parent.document.all.TB_iframeContent.style.width
                   var bodywidth = IntNum(ifwidth.replace('px', ''), 400) -30
                    if(ts.p.width>bodywidth) ts.p.width=bodywidth
                    tblwidth = ts.p.tableWidth < bodywidth ? bodywidth : ts.p.tableWidth;
                } else {
                    var bodywidth = document.body.clientWidth - 10
                    var scrollHeight = document.body.scrollHeight
                    var clientHeight = document.body.clientHeight
                    var shp = scrollHeight + 54 + ts.p.height;
                    if (clientHeight < shp) {
                        if ($("body").find(".grid_table_list_style").length > 1) {
                            ts.p.width = bodywidth
                        } else {
                            ts.p.width = bodywidth - 17
                        }
                    } else {
                        ts.p.width = bodywidth
                    }
                ...
                try {
                    ...
                    $.each(this.p.treeReader, function(i, n) {
                        if (n) {
                            ...
                            ts.p.colModel.push({name:n,width:1,hidden:true,align:"center",sortable:false,resizable:false,hidedlg:true,editable:true,search:false});
                        }
                    });
                } catch (_) {
                }
            ...
            if (this.p.subGrid) {
                ...
                this.p.colModel.unshift({name:'subgrid',width:1,sortable: false,resizable:false,hidedlg:true,search:false});
            }
            if (this.p.multiselect) {
                this.p.colNames.unshift("<input id='cb_jqg' class='cbox' type='checkbox'/>");
                this.p.colModel.unshift({name:'cb',width:1,sortable:false,resizable:false,align:"center",hidedlg:true,search:false});
            }
            var xReader = {
                root: "rows",
                row: "row",
            ...
            $.each(ts.p.colModel, function(i) {
                this.width = IntNum(this.width, 150);
            });
            if (ts.p.width) {
                setColWidth();
            }
            ...
            $(trow).css("height", ts.p.titleHight + "px");
            ...
            $("tr:first th", thead).each(function (j) {
                ...
                temnum = temnum + 1
                if (ts.p.colModel.length == temnum) {
                    var wid = w + 30
                    $(this).css("width", wid + "px").prepend(res);
                } else {
                    var wid = w
                    if ($.browser.mozilla) {
                        wid = wid + 1
            ...
            $("tbody tr:first td", ts).each(function(ii) {
             ...
             //   if (self.parent != undefined && self.parent.document.all != undefined&& self.parent.document != undefined&& self.parent.document.all.TB_iframeContent != undefined) {
             //       var mt=0;
                    //if(ii==0)mt=1
           //        $(this).css({width:(w+mt) + "px",height:"0px","border-right":"1px solid #D4D0C8"});
             //   }else{
                   $(this).css({width:w + "px",height:"0px","border-right":"1px solid #D4D0C8"});
            //    }
            ...
            $(grid.hDiv)
                    .css({ width: ts.p.width + "px", overflow: "hidden",'border-left':"1px solid #D4D0C8" ,'border-right':"1px solid #D4D0C8"})
            ...
            if (ts.p.pager) {
                if (typeof ts.p.pager == "string") {
                    if (ts.p.pager.substr(0, 1) != "#") ts.p.pager = "#" + ts.p.pager;
                }
                if ($(ts.p.pager).hasClass("scroll")) {
                    $(ts.p.pager).css({ width: ts.p.width + "px",height:ts.p.pagerHeight + 'px', overflow: "hidden"}).show();
         ...
         ///   alert(ts.p.width)
        //    alert(ts.p.tableWidth)
            var xfl = (ts.p.width < ts.p.tableWidth) ? "auto" : "hidden";
            $(grid.bDiv).addClass("grid_bdiv")
                    .scroll(function (e) {grid.scrollGrid();})
                    .css({ height: ts.p.height + (isNaN(ts.p.height) ? "" : "px"), padding: "0px", margin: "0px", overflow: ofl2,width: (ts.p.width+1) + "px"}).css({"overflow-x":xfl,'border-left':"1px solid #D4D0C8" ,'border-right':"1px solid #D4D0C8"})
                    .append(this);
            $("table:first", grid.bDiv).css({width:1 + "px"});
            if (isMSIE) {
                ...
                if (ts.p.treeGrid) {
                    $(grid.bDiv).css("position", "relative");
                }
            } else {
                if (ts.p.multikey) {
            ...
            if (ts.p.caption) {
                ...
                if (ts.p.hidegrid === true) {
                    ...
                    if (hg) {
                        $(".HeaderButton", grid.cDiv).trigger("click");
                        ts.p.datatype = "local";
                    }
                }
            } else {
            ...
            $(document).mouseup(function (e) {
                if (grid.resizing) {
                    ...
                    if (grid.newWidth && ts.p.forceFit === false) {
                        ...
                        if (ts.p.caption) {
                            $(grid.cDiv).width(gwdt);
                        }
                        if (ts.p.toolbar[0]) {
                            $(grid.uDiv).width(gwdt);
                        }
            ...
            ts.formatter = function (elem, row, cellval, colpos, act) {
                formatter(elem, row, cellval, colpos, act);
            };
            $.extend(grid, {populate : function() {
                populate();
            }});
            this.grid = grid;
            ...
            var initOverX = $(grid.bDiv).css('overflow-x')
            var OVER_X = false;
            var scrollHeight = document.body.scrollHeight
            var clientHeight = document.body.clientHeight
            ...
            function reload(){
              var bodywidth = document.body.clientWidth - 10
               if (self.parent != undefined && self.parent.document != undefined&& self.parent.document.all != undefined&& self.parent.document.all.TB_iframeContent != undefined) {
                   var ifwidth = self.parent.document.all.TB_iframeContent.style.width
                   bodywidth = IntNum(ifwidth.replace('px', ''), 400) - 30
                }
                    var scrollHeight = document.body.scrollHeight
                    var clientHeight = document.body.clientHeight
                    if (clientHeight < scrollHeight) {
                    ...
                    //  alert(OVER_X);
                    var newwidth2 = bodywidth
                    $(grid.hDiv).css({width:newwidth2 + 'px'})
                    $(grid.bDiv).css({width:newwidth2 + 'px'})
                    $(ts.p.pager).css({width:newwidth2 + 'px'})
                    $(this).css({width:1 + 'px'})
                    $(grid.hTable).css({width:1 + 'px'})
                    var overX = false
                    if (bodywidth < ts.p.tableWidth) {
                        $(grid.bDiv).css({'overflow-x':'auto'})
                        overX = true
                        bodywidth = 500
                    } else {
                        $(grid.bDiv).css({'overflow-x':initOverX})
                    }
                    var initwidth = 0;
                    for (var l = 0; l < ts.p.colModel.length; l++) {
                        if (!ts.p.colModel[l].hidden) {
                    ...
                    //                    alert(ts.p.width)
                    //     alert(bodywidth);
                    var tblwidth = ts.p.tableWidth < bodywidth ? bodywidth : ts.p.tableWidth
                    //    alert(tblwidth)
                    //                    var newwidth = bodywidth - 10
                    //                    var thwide = newwidth / 5
                    var thnum = $(grid.hTable).find("thead>tr>th").length
                    //    alert(thnum);
                    var overY = false;
                    var relline = $(this).getDataIDs().length
                    if (relline > 0) {
                        var height = parseInt($(grid.bDiv).css('height').replace('px', ''))
                        var lh = parseInt($(".jqgrow").css('height').replace('px', ''))
                        if (isNaN(lh)) {
                            lh = 23
                        }
                        var maxline = height / lh
                        overY = (maxline <= relline)
                    }
                //alert(overY);
                    // if (tblwidth != initwidth) {
                    $(grid.hTable).find("thead>tr>th").each(function (j) {
                        var thiswid = Math.round(tblwidth / initwidth * ts.p.colModel[j].width);
                        //   alert(thiswid);
                        if (j == (thnum - 1)) {
                            if (overY) {
                                $(this).css({width:(thiswid + 17) + 'px'})
                            } else {
                                $(this).css({width:thiswid + 'px'})
                            }
                        } else {
                            $(this).css({width:thiswid + 'px'})
                        }
                    });
                    $(grid.bDiv).find("#_empty>td").each(function (j) {
                        var thiswid = Math.round(tblwidth / initwidth * ts.p.colModel[j].width);
                        if (j == (thnum - 1)) {
                            $(this).css({width:thiswid + 'px'})
                        } else {
