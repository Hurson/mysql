...
function dhtmlXTabBar(parentObject,mode,height){
	...
	this._styles={
		...
		"dhx_skyblue":{ left:3, right:3, select_shift:1, select_top:0, margin:-1, offset:5 , data_color:"white", hover:true }
	};
	...
	this.setStyle("default"); this.skin = "";
...
*/
	...
	addTab:function(id, text, size, position, row){  
		if (!this.skin && dhtmlx.skin) this.setSkin(dhtmlx.skin);
		row=row||0;
//#multiline:23052006{			
...
*/
	...
	_goToAny:function(tab,mode,prev){
    	if (this._lastActive==tab){
        	if (mode){
                if(prev) this.goToPrevTab();
...
//    	var thml="<span>"+text+"</span>";
		var thml="<span>"+text+"</span><div class='tab_left'></div><div class='tab_right'></div><div class='tab_main'></div>";
    	if (this._close) thml+="<IMG style='"+this._pxc+":4px; "+this._py+":4px; position:absolute;' src='"+this.imgUrl+this.skin+"/close.png' >"
    	tab.innerHTML=thml;
