...
$(function(){   
    ...
    var ployList="{'list':[";
     <c:choose>
		<c:when test="${!empty ployList}">
			<c:forEach var="ploy" items="${ployList}" varStatus="status">
				ployList+="{'ployOrder':'${status.count}','ployCode':'${ploy.ployId}','ployName':'${ploy.ployName}'}";
				<c:choose>
	...
	ployList+="]}";
	var ployListObject=eval("("+ployList+")");
    $("#searchPloySubmit").click(function(){
     	var ployName = $("#ployName").val();
     	ployName = encodeURI(ployName);   
		ployName = encodeURI(ployName);
     	url='queryPloy.do?method=queryPloy&ployName='+ployName;
     	accessUrl(url);
     	return;
     });
     $("#choosePloyButton").click(function(){
     
     	var alreadyChoose = "";
	    $("input[name='ployCheckBox']").each(function(){
	        if($(this).is(':checked')){
	    ...
	    if($.isEmptyObject(alreadyChoose)){
	    	...
	    	$(alreadyChooseColumn).each(function(index,itemOut){
	    		if(!$.isEmptyObject(itemOut)){
	       			$(ployListObject.list).each(function(index,itemInner){
	       				if((!$.isEmptyObject(itemOut))&&(itemOut==itemInner.ployCode)){
	 		...
	 		parent.showChooseList4Page(1,13);
	    }
		...
		<td style="padding:2px;">
			<table cellpadding="0" cellspacing="1" width="100%" class="tablea">
				...
				<tr>
					...
					<td class="td_input">
						<input id="ployName" name="ployName" class="e_input" type="text" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${ployName}" maxlength="80"/>
					</td>
				</tr>
				<tr>
					<td class="formBottom" colspan="99">
						<input name="searchPloySubmit" id="searchPloySubmit" type="button" title="查看" class="b_search" value=""/>
					</td>
				</tr>
			</table>
		</td>
			...
			class="taba_right_list" id="bm">
					...
					<c:choose>
						<c:when test="${!empty ployList}">
							<c:forEach var="ploy" items="${ployList}" varStatus="status">
								<tr>
									<td align="center" height="26"><input type="radio" id="ployCheckBox" name="ployCheckBox" value="${ploy.ployId}"/></td>
						...
						<td height="34" colspan="12"
							...
							<c:choose>
								<c:when test="${page.pageNo==1&&page.totalPage!=1}">
									<a href="queryPloy.do?pageNo=${page.pageNo+1 }&ployName=${ployName}">下一页</a>&nbsp;&nbsp;
									<a href="queryPloy.do?pageNo=${page.totalPage}&ployName=${ployName}">末页</a>&nbsp;&nbsp;
								</c:when>
								<c:when test="${page.pageNo==page.totalPage&&page.totalPage!=1 }">
									<a href="queryPloy.do?pageNo=1&ployName=${ployName}">首页</a>&nbsp;&nbsp;
									<a href="queryPloy.do?pageNo=${page.pageNo-1 }&ployName=${ployName}">上一页</a>&nbsp;&nbsp;
								</c:when>
					...
					<tr>
						<td class="formBottom" colspan="99" style="text-align: right;">
							 <input id="choosePloyButton" name="choosePloyButton" type="button" title="确定" class="b_add" value="" onfocus=blur()/>
						</td>
					</tr>
				</table>
			</div>
