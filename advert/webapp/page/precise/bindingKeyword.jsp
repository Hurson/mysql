...
$(function(){   
	...
	keywordList+="]}";
	var keywordListObject=eval("("+keywordList+")");
    $("#searchKeywordSubmit").click(function(){
     	var keyword = $("#keyword").val();
     	keyword = encodeURI(keyword);   
		keyword = encodeURI(keyword);
     	url='queryKeyword.do?method=queryKeyword&keyword='+keyword;
     	accessUrl(url);
     	return;
     });
     $("#chooseKeywordButton").click(function(){
	    ...
	    if($.isEmptyObject(alreadyChoose)){
	    	...
	    	$(alreadyChooseColumn).each(function(index,itemOut){
	    		if(!$.isEmptyObject(itemOut)){
	       			$(keywordListObject.list).each(function(index,itemInner){
	       				if((!$.isEmptyObject(itemOut))&&(itemOut==itemInner.keyword)){
	 		...
	 		parent.showChooseList4Page(1,8);
	    }
	...
	<tr>
		<td style="padding:2px;">
			<table cellpadding="0" cellspacing="1" width="100%" class="tablea">
				<tr>
					<td class="formTitle" colspan="99"><span>·关键字查询</span></td>
				</tr>
				<tr>
					<td class="td_label">关键字：</td>
					<td class="td_input">
						<input id="keyword" name="keyword" class="e_input" type="text" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${keyword}" maxlength="80"/>
					</td>
				</tr>
				<tr>
					<td class="formBottom" colspan="99">
						<input name="searchKeywordSubmit" id="searchKeywordSubmit" type="button" title="查看" class="b_search" value=""/>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		<td style="padding:2px;">
			...
			class="taba_right_list" id="bm">
					<tr>
						<td colspan="23" style="padding-left: 8px; background: url(<%=path %>/images/menu_righttop.png) repeat-x; text-align: left; height: 26px;"><span>·关键字列表</span></td>
					</tr>
					<tr>
					...
					<c:choose>
						<c:when test="${!empty keywordList}">
							<c:forEach var="keyword" items="${keywordList}" varStatus="status">
								<tr>
									<td align="center" height="26"><input type="checkbox" id="keywordCheckBox${keyword.id}" name="keywordCheckBox" value="${keyword.key}"/></td>
						...
						<td height="34" colspan="12"
							...
							<c:choose>
								<c:when test="${page.pageNo==1&&page.totalPage!=1}">
									<a href="queryKeyword.do?pageNo=${page.pageNo+1 }&keyword=${keyword}">下一页</a>&nbsp;&nbsp;
									<a href="queryKeyword.do?pageNo=${page.totalPage}&keyword=${keyword}">末页</a>&nbsp;&nbsp;
								</c:when>
								<c:when test="${page.pageNo==page.totalPage&&page.totalPage!=1 }">
									<a href="queryKeyword.do?pageNo=1&keyword=${keyword}">首页</a>&nbsp;&nbsp;
									<a href="queryKeyword.do?pageNo=${page.pageNo-1 }&keyword=${keyword}">上一页</a>&nbsp;&nbsp;
								</c:when>
					...
					<tr>
						<td class="formBottom" colspan="99" style="text-align: right;">
							 <input id="chooseKeywordButton" name="chooseKeywordButton" type="button" title="确定" class="b_add" value="" onfocus=blur()/>
						</td>
					</tr>
				</table>
			</div>
