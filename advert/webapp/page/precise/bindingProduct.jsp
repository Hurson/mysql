...
$(function(){   
     ...
     <c:choose>
		<c:when test="${!empty productList}">
			<c:forEach var="product" items="${productList}" varStatus="status">
				productList+="{'productOrder':'${status.count}','productCode':'${product.productCode}','productName':'${product.productName}'}";
				<c:choose>
	...
	productList+="]}";
	var productListObject=eval("("+productList+")");
    $("#searchProductSubmit").click(function(){
     	var productName = $("#productName").val();
     	productName = encodeURI(productName);   
		productName = encodeURI(productName);
     	url='queryProduct.do?method=queryProduct&productName='+productName;
     	accessUrl(url);
     	return;
     });
     $("#chooseProductButton").click(function(){
	    ...
	    if($.isEmptyObject(alreadyChoose)){
	    	...
	    	$(alreadyChooseColumn).each(function(index,itemOut){
	    		if(!$.isEmptyObject(itemOut)){
	       			$(productListObject.list).each(function(index,itemInner){
	       				if((!$.isEmptyObject(itemOut))&&(itemOut==itemInner.productCode)){
	 		...
	 		parent.showChooseList4Page(1,6);
	    }
	...
	<tr>
		<td style="padding:2px;">
			<table cellpadding="0" cellspacing="1" width="100%" class="tablea">
				<tr>
					<td class="formTitle" colspan="99"><span>·产品查询</span></td>
				</tr>
				<tr>
					<td class="td_label">产品名称：</td>
					<td class="td_input">
						<input id="productName" name="productName" class="e_input" type="text" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${productName}" maxlength="80"/>
					</td>
				</tr>
				<tr>
					<td class="formBottom" colspan="99">
						<input name="searchProductSubmit" id="searchProductSubmit" type="button" title="查看" class="b_search" value=""/>
					</td>
				</tr>
			</table>
		</td>
			...
			class="taba_right_list" id="bm">
					...
					<c:choose>
						<c:when test="${!empty productList}">
							<c:forEach var="product" items="${productList}" varStatus="status">
								<tr>
									<td align="center" height="26"><input type="checkbox" id="productCheckBox" name="productCheckBox" value="${product.productCode}"/></td>
						...
						<td height="34" colspan="12"
							...
							<c:choose>
								<c:when test="${page.pageNo==1&&page.totalPage!=1}">
									<a href="queryProduct.do?pageNo=${page.pageNo+1 }&productName=${productName}">下一页</a>&nbsp;&nbsp;
									<a href="queryProduct.do?pageNo=${page.totalPage}&productName=${productName}">末页</a>&nbsp;&nbsp;
								</c:when>
								<c:when test="${page.pageNo==page.totalPage&&page.totalPage!=1 }">
									<a href="queryProduct.do?pageNo=1&productName=${productName}">首页</a>&nbsp;&nbsp;
									<a href="queryProduct.do?pageNo=${page.pageNo-1 }&productName=${productName}">上一页</a>&nbsp;&nbsp;
								</c:when>
					...
					<tr>
						<td class="formBottom" colspan="99" style="text-align: right;">
							 <input id="chooseProductButton" name="chooseProductButton" type="button" title="确定" class="b_add" value="" onfocus=blur()/>
						</td>
					</tr>
				</table>
			</div>
