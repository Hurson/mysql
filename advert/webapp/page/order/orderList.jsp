...
<input type="hidden" id="pageSize" name="page.pageSize" value="${page.pageSize}"/>
	<table cellspacing="1" class="searchList">
	  ...
	  <tr>
	    <td class="searchCriteria">
	      <span>合同名称：</span><input type="text" id="contractName" name="order.contractName" value="${order.contractName }" />
	      <span>广告位名称：</span><input type="text" id="positionName" name="order.positionName" value="${order.positionName}" />
	      <span>策略名称：</span><input type="text" id="ployName" name="order.ployName" value="${order.ployName }" />
	      <span>状态：</span>
    ...
    <td class="searchCriteria">
      <span style="width: 60px;text-align:right;">开始日期：</span>
	   	<input id="startDateStr" name="order.startDateStr" value="${order.startDateStr}" type="text" readonly="readonly" style="width:  100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
...
<table cellspacing="1" class="searchList" >
	...
	<s:iterator value="page.dataList" status="status" var="order">
		<tr <s:if test="#status.index%2==1">class="sec"</s:if>
			...
			<td><a href="getOrderForUpdate2.do?order.id=<s:property value='#order.id' />"><s:property value="#order.orderCode" /></a></td>
			<td><s:property value="#order.contractName" /></td>
			<td><s:property value="#order.positionName" /></td>
			<td><s:property value="#order.ployName" /></td>
			<td><s:date name="#order.startTime" format="yyyy-MM-dd" />~<s:date name="#order.endTime" format="yyyy-MM-dd" /></td>
			<td>
  ...
  <tr>
    <td colspan="7">
        ...
        <input type="button" value="添加" class="btn" onclick="window.location.href='initSave.do'"/>
