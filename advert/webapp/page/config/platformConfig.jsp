...
function deletePositionType(methodName,formName,id,typeCode,typeName){
	var url = 'page/config/removePlatformUrl.do?method=remove';
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tr>
			<td style=" padding:1px;">
				<form action="saveUrl.do?method=saveUrl"  id="addPositionTypeForm" name="addPositionTypeForm" method="post">
					<table cellpadding="0" cellspacing="1" width="100%" class="tablea" >
						<tr>
							<td class="formTitle" colspan="99">
								<span>添加/编辑 投放平台URL</span>
							</td>
						</tr>
						<tr>
							<td class="td_label">平台名称：</td>
							<td class="td_input" colspan="3">
								<input id="idAdd" name="platformConfigBean.id" type="hidden" value="${platformConfigBean.id}"/>
								<input id="typeCodeAdd" name="platformConfigBean.name" type="text" class="e_input" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${platformConfigBean.name}" maxlength="40"/> <font color="red">*</font>		
							</td>
						</tr>
						<tr>
							<td class="td_label">平台URL：</td>
							<td class="td_input" colspan="3">
								<input id="typeNameAdd" name="platformConfigBean.value" type="text" class="e_input" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${platformConfigBean.value}" maxlength="80"/> <font color="red">*</font>		
							</td>
						</tr>
						<tr>
		...
		<tr>
			<td style="padding: 1px;">
				...
				class="taba_right_list" id="bm">
						<tr>
							<td colspan="23" style="padding-left: 8px; background: url(<%=path %>/images/menu_righttop.png) repeat-x; text-align: left; height: 26px;"><span>展示·投放平台配置列表</span></td>
						</tr>
						<tr>
						...
						<c:choose>
							<c:when test="${!empty platformConfigBeans}">
								<c:forEach var="item" items="${platformConfigBeans}" varStatus="status">
									<tr>
									<c:set var="index" value="${index+1 }"/>
										...
										<td>
											<form action="" method="get" id="listPositionTypeOperationForm${status.count}" name="listPositionTypeOperationForm${status.count}">
												<input id="id" name="platformConfigBean.id" type="hidden" value="${item.id}"/>
												<input id="typeCode" name="platformConfigBean.name" type="hidden"  value="${item.name}"/>	
												<input id="typeName" name="platformConfigBean.value" type="hidden"  value="${item.value}"/>	
