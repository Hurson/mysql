...
function deletePositionType(methodName,formName,id,typeCode,typeName){
	var url = 'page/config/deleteInterface.do?method=deleteInterface';
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tr>
			<td style=" padding:1px;">
				<form action="saveOrUpdateInterface.do?method=saveOrUpdateInterface"  id="addPositionTypeForm" name="addPositionTypeForm" method="post">
					<table cellpadding="0" cellspacing="1" width="100%" class="tablea" >
						<tr>
							<td class="formTitle" colspan="99">
								<span>添加/编辑  数据同步接口</span>
							</td>
						</tr>
						<tr>
							<td class="td_label">接口名称：</td>
							<td class="td_input" colspan="3">
								<input id="idAdd" name="interfaceConfigBean.id" type="hidden" value="${interfaceConfigBean.id}"/>
								<input id="typeCodeAdd" name="interfaceConfigBean.name" type="text" class="e_input" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${interfaceConfigBean.name}" maxlength="40"/> <font color="red">*</font>		
							</td>
						</tr>
						<tr>
							<td class="td_label">接口地址：</td>
							<td class="td_input" colspan="3">
								<input id="typeNameAdd" name="interfaceConfigBean.value" type="text" class="e_input" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${interfaceConfigBean.value}" maxlength="80"/> <font color="red">*</font>		
							</td>
						</tr>
						<tr>
		...
		<tr>
			<td style="padding: 1px;">
				...
				class="taba_right_list" id="bm">
						<tr>
							<td colspan="23" style="padding-left: 8px; background: url(<%=path %>/images/menu_righttop.png) repeat-x; text-align: left; height: 26px;"><span>展示·同步数据接口列表</span></td>
						</tr>
						<tr>
						...
						<c:choose>
							<c:when test="${!empty interfaceConfigBeans}">
								<c:forEach var="item" items="${interfaceConfigBeans}" varStatus="status">
									<tr>
									<c:set var="index" value="${index+1 }"/>
										...
										<td>
											<form action="" method="get" id="listPositionTypeOperationForm${status.count}" name="listPositionTypeOperationForm${status.count}">
												<input id="id" name="interfaceConfigBean.id" type="hidden" value="${item.id}"/>
												<input id="typeCode" name="interfaceConfigBean.name" type="hidden"  value="${item.name}"/>	
												<input id="typeName" name="interfaceConfigBean.value" type="hidden"  value="${item.value}"/>	
