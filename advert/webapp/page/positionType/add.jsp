	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tr>
			<td style=" padding:1px;">
				<form action="addPositionType.do?method=save"  id="addPositionTypeForm" name="addPositionTypeForm" method="post">
					<table cellpadding="0" cellspacing="1" width="100%" class="tablea" >
						<tr>
							<td class="formTitle" colspan="99">
								<span>添加/编辑 广告位类型</span>
							</td>
						</tr>
						<tr>
							<td class="td_label">类型编码：</td>
							<td class="td_input" colspan="3">
								<input id="idAdd" name="advertPositionType.id" type="hidden" value="${advertPositionType.id}"/>
								<input id="typeCodeAdd" name="advertPositionType.typeCode" type="text" class="e_input" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${advertPositionType.typeCode}" maxlength="40"/> <font color="red">*</font>		
							</td>
						</tr>
						<tr>
							<td class="td_label">类型名称：</td>
							<td class="td_input" colspan="3">
								<input id="typeNameAdd" name="advertPositionType.typeName" type="text" class="e_input" onfocus="this.className='e_inputFocus'" onblur="this.className='e_input'" value="${advertPositionType.typeName}" maxlength="80"/> <font color="red">*</font>		
							</td>
						</tr>
						<tr>
							...
							<c:when test="${!empty advertPositionTypeList}">
								<c:forEach var="advertPositionType" items="${advertPositionTypeList}" varStatus="status">
									<tr>
										...
										<td>${advertPositionType.typeCode}</td>
										<td>${advertPositionType.typeName}</td>
										<td>
											<form action="" method="get" id="listPositionTypeOperationForm${status.count}" name="listPositionTypeOperationForm${status.count}">
												<input id="id" name="advertPositionType.id" type="hidden" value="${advertPositionType.id}"/>
												<input id="typeCode" name="advertPositionType.typeCode" type="hidden"  value="${advertPositionType.typeCode}"/>	
												<input id="typeName" name="advertPositionType.typeName" type="hidden"  value="${advertPositionType.typeName}"/>	

												<input name="deletePositionType${status.count}" id="deletePositionType${status.count}" type="button" class="button_delete" value="" title="删除" onClick="operation('deletePositionType','#listPositionTypeOperationForm${status.count}','${advertPositionType.id}')" />
												<input name="modifyPositionType${status.count}" id="modifyPositionType${status.count}"  type="button" class="button_halt" value="" title="修改" onClick="operation('modifyPositionType','#listPositionTypeOperationForm${status.count}','${advertPositionType.id}','${advertPositionType.typeCode}','${advertPositionType.typeName}')" />
											</form>
										</td>
									</tr>
								</c:forEach>
