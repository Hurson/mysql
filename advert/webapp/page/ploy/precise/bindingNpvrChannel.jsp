...
{
	var optionSelelct =window.dialogArguments.document.getElementsByName("preciseUiBean.playbackChannelId");
    var flag=false;
...
function selectData(){
    	...
    	for (var i=0;i<strdatas.length;i++)
    	{
	       ...
	       if (flag==false)
	        {
	        	window.dialogArguments.add_Npvr(dataPropertys[0],dataPropertys[1]);
...
<body class="mainBody">
 <form action="<%=path %>/page/precise/queryNpvrChannel.do" method="post" id="queryForm">
 <s:set name="page" value="%{pageNpvrChannel}" />
		 <input type="hidden" id="pageNo" name="pageNpvrChannel.pageNo" value="${page.pageNo}"/>
		 <input type="hidden" id="pageSize" name="pageNpvrChannel.pageSize" value="${page.pageSize}"/>
 <input type="hidden" id="ployId" name="ployId" value="${ployId}"/>
 
<div class="search">
<div class="path">首页 >> 投放策略管理 >> 精准维护>>选择回放频道</div>
...
<table cellspacing="1" class="searchList">
  ...
  <tr>
    <td class="searchCriteria">
      <span>频道名称：</span><input onkeypress="return validateSpecialCharacter();" type="text" name="npvrChannel.channelName" id="npvrChannel.channelName" value="${channel.channelName}"/>
...
<table cellspacing="1" class="searchList">
    <tr class="title">
        ...
        <td width="30%" align="center">ServiceId</td>
   				 ...
   				 <c:if test="${pageNpvrChannel.dataList != null && fn:length(pageNpvrChannel.dataList) > 0}">
                    <c:forEach items="${pageNpvrChannel.dataList}" var="npvrInfo" varStatus="pl">
                        <tr <c:if test="${pl.index%2==1}">class="sec"</c:if>>
                            <td>
                                <input id="data_${npvrInfo.serviceId}" type="checkbox" value="<c:out value='${npvrInfo.serviceId}_${npvrInfo.channelName}'/>" name="dataIds"/>
                             </td>
                            <td width="30%" >
                               ${npvrInfo.channelName}
                            </td>
                            <td width="30%" align="center">
                         
                              ${npvrInfo.channelCode}
							</td>
