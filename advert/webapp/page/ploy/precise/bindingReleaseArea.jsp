...
{
	var optionSelelct =window.dialogArguments.document.getElementById("precise.userArea");
    var flag=false;
...
function selectData(){
    	...
    	for (var i=0;i<strdatas.length;i++)
    	{
	       ...
	       if (flag==false)
	        {
	        	window.dialogArguments.addselectData("precise.userArea",dataPropertys[0],dataPropertys[1]);
	        }
...
<body class="mainBody">
 <form action="<%=path %>/page/precise/queryReleaseArea.do" method="post" id="queryForm">
 <s:set name="page" value="%{pagereleaseArea}" />
		 <input type="hidden" id="pageNo" name="pagereleaseArea.pageNo" value="${page.pageNo}"/>
		 <input type="hidden" id="pageSize" name="pagereleaseArea.pageSize" value="${page.pageSize}"/>
 <input type="hidden" id="ployId" name="ployId" value="${ployId}"/>
  <input type="hidden" id="positionId" name="positionId" value="${positionId}"/>
	
<div class="search">
<div class="path">首页 >> 投放策略管理 >> 精准维护>>选择区域</div>
...
<table cellspacing="1" class="searchList">
  ...
  <tr>
    <td class="searchCriteria">
      <span>区域名称：</span><input onkeypress="return validateSpecialCharacter();" type="text" name="releaseArea.areaName" id="releaseArea.areaName" value="${releaseArea.areaName}"/>
...
<table cellspacing="1" class="searchList">
    <tr class="title">
        ...
        <td width="30%" align="center">价格</td>
   				 ...
   				 <c:if test="${pagereleaseArea.dataList != null && fn:length(pagereleaseArea.dataList) > 0}">
                    <c:forEach items="${pagereleaseArea.dataList}" var="areaInfo" varStatus="pl">
                        <tr <c:if test="${pl.index%2==1}">class="sec"</c:if>>
                            <td>
                                <input id="data_${areaInfo.id}" type="checkbox" value="<c:out value='${areaInfo.id}_${areaInfo.areaName}'/>" name="dataIds"/>
                             </td>
                            <td width="30%" >
                               ${areaInfo.areaName}
                            </td>
