...
{
	var optionSelelct =window.dialogArguments.document.getElementById("precise.assetId");
    var flag=false;
...
function selectData(){
    	...
    	for (var i=0;i<strdatas.length;i++)
    	{
	       ...
	       if (flag==false)
	        {
	        	window.dialogArguments.addselectData("precise.assetId",dataPropertys[0],dataPropertys[1]);
	        }
...
<body class="mainBody">
 <form action="<%=path %>/page/precise/queryAsset.do" method="post" id="queryForm">
 <s:set name="page" value="%{pageAsset}" />
		 <input type="hidden" id="pageNo" name="pageAsset.pageNo" value="${page.pageNo}"/>
		 <input type="hidden" id="pageSize" name="pageAsset.pageSize" value="${page.pageSize}"/>
 <input type="hidden" id="ployId" name="ployId" value="${ployId}"/>
 
<div class="search">
<div class="path">首页 >> 投放策略管理 >> 精准维护>>选择影片</div>
...
<table cellspacing="1" class="searchList">
  ...
  <tr>
    <td class="searchCriteria">
      <span>影片名称：</span><input onkeypress="return validateSpecialCharacter();" type="text" name="asset.assetName" id="asset.assetName" value="${asset.assetName}"/>
...
<table cellspacing="1" class="searchList">
    <tr class="title">
        ...
        <td width="30%" align="center">影片名称</td>
        <td width="30%" align="center">导演</td>
        <td width="30%" align="center">演员</td>
     
    </tr>
   				 <c:if test="${pageAsset.dataList != null && fn:length(pageAsset.dataList) > 0}">
                    <c:forEach items="${pageAsset.dataList}" var="assetInfo" varStatus="pl">
                        <tr <c:if test="${pl.index%2==1}">class="sec"</c:if>>
                            <td>
                                <input id="data_${assetInfo.id}" type="checkbox" value="<c:out value='${assetInfo.id}_${assetInfo.assetName}'/>" name="dataIds"/>
                             </td>
                            <td width="30%" >
                               ${assetInfo.assetName}
                            </td>
