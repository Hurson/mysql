...
{
	var optionSelelct =window.dialogArguments.document.getElementsByName("preciseUiBean.userlevels");
    var flag=false;
...
function selectData(){
    	...
    	for (var i=0;i<strdatas.length;i++)
    	{
	       ...
	       if (flag==false)
	        {
	        	window.dialogArguments.add_Level(dataPropertys[0],dataPropertys[1]);
	        }
...
<body class="mainBody">
 ...
 <s:set name="page" value="%{pageuserRank}" />
		 <input type="hidden" id="pageNo" name="pageuserRank.pageNo" value="${page.pageNo}"/>
		 <input type="hidden" id="pageSize" name="pageuserRank.pageSize" value="${page.pageSize}"/>
...
<table cellspacing="1" class="searchList">
    <tr class="title">
        ...
        <td width="30%" align="center">级别描述</td>
   				 ...
   				 <c:if test="${pageuserRank.dataList != null && fn:length(pageuserRank.dataList) > 0}">
                    <c:forEach items="${pageuserRank.dataList}" var="userbankInfo" varStatus="pl">
                        <tr <c:if test="${pl.index%2==1}">class="sec"</c:if>>
                            <td>
                                <input id="data_${userbankInfo.id}" type="checkbox" value="<c:out value='${userbankInfo.id}_${userbankInfo.userRankName}'/>" name="dataIds"/>
                             </td>
                            <td width="30%" >
