...
{
	var optionSelelct =window.dialogArguments.document.getElementsByName("preciseUiBean.userindustrys");
    var flag=false;
...
function selectData(){
    	...
    	for (var i=0;i<strdatas.length;i++)
    	{
	       ...
	       if (flag==false)
	        {
	        	window.dialogArguments.add_Ind(dataPropertys[0],dataPropertys[1]);
	        }
...
<body class="mainBody">
 <form action="<%=path %>/page/precise/queryUserIndustry.do" method="post" id="queryForm">
 <s:set name="page" value="%{pageuserIndustry}" />
		 <input type="hidden" id="pageNo" name="pageuserIndustry.pageNo" value="${page.pageNo}"/>
		 <input type="hidden" id="pageSize" name="pageuserIndustry.pageSize" value="${page.pageSize}"/>
 <input type="hidden" id="ployId" name="ployId" value="${ployId}"/>
 
<div class="search">
<div class="path">首页 >> 投放策略管理 >> 精准维护>>选择行业</div>
...
<table cellspacing="1" class="searchList">
  ...
  <tr>
    <td class="searchCriteria">
      <span>行业名称：</span><input onkeypress="return validateSpecialCharacter();" type="text" name="userIndustry.userIndustryCategoryValue" id="userIndustry.userIndustryCategoryValue" value="${userIndustry.userIndustryCategoryValue}"/>
...
<table cellspacing="1" class="searchList">
    <tr class="title">
        ...
        <td width="30%" align="center">行业名称</td>
        <td width="30%" align="center">行业编码</td>
        <td width="30%" align="center">价格</td>
     
    </tr>
   				 <c:if test="${pageuserIndustry.dataList != null && fn:length(pageuserIndustry.dataList) > 0}">
                    <c:forEach items="${pageuserIndustry.dataList}" var="indInfo" varStatus="pl">
                        <tr <c:if test="${pl.index%2==1}">class="sec"</c:if>>
                            <td>
                                <input id="data_${indInfo.id}" type="checkbox" value="<c:out value='${indInfo.id}_${indInfo.userIndustryCategoryValue}'/>" name="dataIds"/>
                             </td>
                            <td width="30%" >
                               ${indInfo.userIndustryCategoryValue}
                            </td>
                            <td width="30%" align="center">
                         
                              ${indInfo.userIndustryCategoryCode}
							</td>
