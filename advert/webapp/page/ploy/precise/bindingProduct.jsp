...
{
	//var optionSelelct =window.dialogArguments.document.getElementById("precise.productId");
	var optionSelelct =window.dialogArguments.document.getElementsByName("preciseUiBean.productId");
...
function selectData(){
    	...
    	for (var i=0;i<strdatas.length;i++)
    	{
	       ...
	       if (flag==false)
	        {
	        	window.dialogArguments.add_Product(dataPropertys[0],dataPropertys[1]);
	        }
...
<body class="mainBody">
 <form action="<%=path %>/page/precise/queryProduct.do" method="post" id="queryForm">
 <s:set name="page" value="%{pageProduct}" />
		 <input type="hidden" id="pageNo" name="pageProduct.pageNo" value="${page.pageNo}"/>
		 <input type="hidden" id="pageSize" name="pageProduct.pageSize" value="${page.pageSize}"/>
...
<div class="search">
<div class="path">首页 >> 投放策略管理 >> 精准维护>>选择产品</div>
...
<table cellspacing="1" class="searchList">
  ...
  <tr>
    <td class="searchCriteria">
      <span>产品名称：</span><input onkeypress="return validateSpecialCharacter();" type="text" name="product.productName" id="product.productName" value="${product.productName}"/>
...
<table cellspacing="1" class="searchList">
    <tr class="title">
        ...
        <td width="30%" align="center">产品描述</td>
        <td width="30%" align="center">价格</td>
     
    </tr>
   				 <c:if test="${pageProduct.dataList != null && fn:length(pageProduct.dataList) > 0}">
                    <c:forEach items="${pageProduct.dataList}" var="productInfo" varStatus="pl">
                        <tr <c:if test="${pl.index%2==1}">class="sec"</c:if>>
                            <td>
                                <input id="data_${productInfo.id}" type="checkbox" value="<c:out value='${productInfo.id}_${productInfo.productName}'/>" name="dataIds"/>
                             </td>
                            <td width="30%" >
