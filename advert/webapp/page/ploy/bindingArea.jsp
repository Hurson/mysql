     ...
     <c:choose>
		<c:when test="${!empty areaList}">
			<c:forEach var="advertArea" items="${areaList}" varStatus="status">
				areaList+="{'areaOrder':'${status.count}','id':'${advertArea.id}','areaName':'${advertArea.areaName}'}";
				<c:choose>
		...
		<td style="padding:2px;">
					...
					<c:choose>
						<c:when test="${!empty areaList}">
							<c:forEach var="areas" items="${areaList}" varStatus="status">
								<tr>
									<td align="center" height="26"><input type="checkbox" id="areaCheckbox${areas.id}" name="areaCheckbox" value="${areas.id}"/></td>
			...
			<tr>
						<td class="formBottom" colspan="99" style="text-align: right;">
							 <input id="chooseAreaButton" name="chooseAreaButton" type="button" title="确定" class="b_add" value="" onfocus=blur()/>
						</td>
					</tr>		
