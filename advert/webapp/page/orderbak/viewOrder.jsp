...
<body
	onload='init("${order.coordinate }",${order.isHD },${order.isLoop},"${order.positionSize}",${ployMaterialJson },${materialJson },"${order.fontSize }","${order.rollSpeed }","${order.fontColor }");'>
	...
	<tr>
		...
		<table cellpadding="0" cellspacing="1" width="100%" class="tablea">
			...
			<tr>
				...
				<td class="td_inputz"><span class="e_input_text">${order.endTime
				}</span></td>

			</tr>
			<tr>
				<td class="td_labelz">订单描述：</td>
				<td class="td_inputz" colspan="3"><textarea class="e_textarea"
			...
			<tr>
				...
				<table cellpadding="0" cellspacing="1" width="100%" class="tablea">
					<tr>
						...
						<ul>
							<li>已选择策略</li>
							<div id="selPloy"><span id="selPloyName"> <c:if
								test="${(order.isAdd==0||order.isLoop==1)&&order.isInstream==0 }">
								<a href="javascript:showPloyMaterial();">${order.ployName }</a>
							</c:if> <c:if
								test="${(order.isAdd==1&&order.isLoop==0)||order.isInstream!=0}">
								${order.ployName }
							</c:if> </span> <br />
							&nbsp;&nbsp;&nbsp;&nbsp; <span id="mp0"> <c:set var="i"
								value="0" /> <c:forEach items="${order.ployMaterials }"
								var="pm">
								<c:if test="${i>0}">,</c:if>
								<c:if
									test="${(order.isAdd==1&&order.isLoop==0)||order.isInstream!=0 }">
							...
							<div id="selPrecise"><c:set var="j" value="0" /> <c:forEach
								...
								<c:if
									test="${(order.isAdd==0||order.isLoop==1)&&order.isInstream==0 }">
									<a href="javascript:showPreciseMaterial(${ms.pId });">${ms.preciseName
									} </a>
								</c:if>
								<c:if
									test="${(order.isAdd==1&&order.isLoop==0)||order.isInstream!=0 }">
								...
								<c:forEach items="${ms.material }" var="prm">
									...
									<c:if
										test="${(order.isAdd==1&&order.isLoop==0)||order.isInstream!=0 }">
										<a
											href="javascript:showMaterial('${prm.path}','${prm.content}',${prm.type})">${prm.name
										}</a>
									</c:if>
									<c:if
										test="${(order.isAdd==0||order.isLoop==1)&&order.isInstream==0 }">
					...
					<tr>
						<td class="formBottom" colspan="99"><!--<input type="button" title="大图预览" onclick="" class="b_preview" value=""
							...
							<c:when test="${order.state=='3'}">
								【未发布订单】审核不通过&nbsp;&nbsp;&nbsp;&nbsp;
								<a href='javascript:showOpinion();'>查看审核意见</a>
							</c:when>
						...
						</c:choose> </span> <input type="button" title="返回" onclick="history.go(-1)"
							class="b_back" value="" onfocus="blur()" /></td>
					</tr>
				</table>
				</td>
			</tr>
...
<table cellpadding="0" cellspacing="1" width="100%"
	...
	<tr>
		...
		<div class="buttons">
		<a href="#" onclick="hideOpinion();">关闭</a></div>
		</td>
	</tr>
</table>
</div>
