...
<script type="text/javascript" src="<%=path%>/js/order/viewOrder.js"></script>
<script type="text/javascript" src="<%=path%>/js/order/checkOrder.js"></script>
<title>订单管理</title>
<style>
.treven {
	background: #FFFFFF;
...
<body
	onload='init("${order.coordinate }",${order.isHD },${order.isLoop},"${order.positionSize}",${ployJson },${ployMaterialJson },${materialJson },"${order.fontSize }","${order.rollSpeed }","${order.fontColor }");'>
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	...
	<tr>
		...
		<table cellpadding="0" cellspacing="1" width="100%" class="tablea">
			<tr>
				<td class="formTitle" colspan="99"><span>·查看订单</span></td>
			</tr>
			<tr>
				<td class="td_labelz">订单号：</td>
				<td class="td_inputz"><span class="e_input_text">${order.orderNo
				}</span></td>
			...
			<tr>
				...
				<td class="td_inputz"><span class="e_input_text">${order.endTime }</span></td>
				...
				<table cellpadding="0" cellspacing="1" width="100%" class="tablea">
					<tr>
						...
						<ul>
							<li>已选择策略</li>
							<div id="selPloy">
								<a href="javascript:showPloyInfo();">${order.ployName }</a>
							 <br />
							&nbsp;&nbsp;&nbsp;&nbsp; <span id="mp0">
							 <c:set var="i" value="0" />
							...
							<div id="selPrecise"><c:set var="j" value="0" /> 
							
							<c:forEach items="${order.materials }" var="ms">
								<c:if test="${j>0}">
									<br />
									<br />
								</c:if>
					...
					<tr>
						<td class="formBottom" colspan="99"><!--<input type="button" title="大图预览" onclick="" class="b_preview" value=""
							onfocus="blur()" /> 
							-->
						<span style="float:left;margin-left:20px;">订单状态：
							<c:choose>
						...
						<a href="javascript:passCheck(${order.id },${order.state },'${order.startTime }','${order.endTime }','${order.ployStartTime }','${order.ployEndTime }',${aheadTime },'${order.modifyTime}',${order.ployId },${order.orderType },0);">审核通过</a>
						<a href="javascript:passCheck(${order.id },${order.state },'${order.startTime }','${order.endTime }','${order.ployStartTime }','${order.ployEndTime }',${aheadTime },'${order.modifyTime}',${order.ployId },${order.orderType },1);">审核不通过</a>
						<input type="button" title="返回"
							onclick="history.go(-1)" class="b_back" value=""
							onfocus="blur()" /></td>
					</tr>
				</table>
				</td>
...
<div id="checkDiv" class="showDiv" style="display:none">
<table cellpadding="0" cellspacing="1" width="100%"
	...
	<tr>
		...
		<td class="td_labelz">审核结果：</td>
		<td class="td_inputz"><span class="e_input_text" id="checkInfo"></span></td>
	</tr>
	<tr>
		<td class="td_labelz">审核意见：</td>
		<td colspan="3" style="background: #ffffff;">
			<textarea style="width:460px;height:100px;font-size: 12px;" id="checkOpinion"></textarea>
	...
	<tr>
		...
		<div class="buttons"><a href="javascript:save(${order.id},${order.orderType });">提交</a>&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:hideDiv();">取消</a></div>
		</td>
	</tr>
</table>
</div>
