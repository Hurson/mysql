...
public class DirFileIndex {
		//临时索引目录
		private Logger logger = Logger.getLogger(DirFileIndex.class);
	    private static DirFileIndex g_fileIndex=null;
		...
		public  DirFileIndex(String indexPath){
		...
		public static DirFileIndex getInstanceFileIndex(String indexPath,Class clazz)
		{
			if (g_fileIndex==null)
			{
				g_fileIndex = new DirFileIndex( indexPath,clazz);
			}
			
			return g_fileIndex;
		}
		public DirFileIndex(String indexPath,Class clazz){
		...
		public void createIndex(List<Object> list,Class clazz) throws IOException, IllegalArgumentException, IllegalAccessException, InvocationTargetException{
		...
		@SuppressWarnings("rawtypes")
		private static void addIndex(IndexWriter writer,List<Object> list,Class clazz) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException, IOException{						
			for(Object obj:list){
...
//			//值较大,适合批量建立索引和更快的搜索
		...
		//创建索引
		@SuppressWarnings({ "unchecked", "rawtypes" })
		public void createCacheIndex(List<Object> list,Class clazz) throws IOException, IllegalArgumentException, IllegalAccessException, InvocationTargetException{
			if(list == null || list.size()<1){				
				return;
			}
			classMap.put(clazz.getName(),clazz.getName());			
			Analyzer analyzer = new StandardAnalyzer(Version.LUCENE_44);
			IndexWriterConfig iwc = new IndexWriterConfig(Version.LUCENE_44, analyzer);
//			//最大缓存文档数  SetMaxBufferedDocs是控制写入一个新的segment前内存中保存的document的数目，设置较大的数目可以加快建索引速度，默认为10。
			iwc.setMaxBufferedDocs(10000);
			LogMergePolicy mergePolicy = new LogByteSizeMergePolicy();
//			//设置segment添加文档(Document)时的合并频率 MergeFactor的默认值是10，
//			//值较小,建立索引的速度就较慢
...
//			//值较大,适合批量建立索引和更快的搜索
		...
		//创建索引
		@SuppressWarnings({ "unchecked", "rawtypes" })
		public void createCacheIndex(List<Object> list,Class clazz,IndexWriter writer) throws IOException, IllegalArgumentException, IllegalAccessException, InvocationTargetException{
			if(list == null || list.size()<1){				
				return;
			}
			classMap.put(clazz.getName(),clazz.getName());			
...
//			}
		...
		private static void addCacheIndex(IndexWriter writer,List<Object> list,Class clazz) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException, IOException{						
			for(Object obj:list){
				Document doc = new Document();
				Method m[] = clazz.getMethods();
